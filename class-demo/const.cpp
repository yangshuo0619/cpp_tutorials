...
int main()
{
   ...
   a1 = &c;   //right;    a1为const int*类型，const在int*左侧，用来修饰指针所指向的变量，即指针指向为常量。所以a1指向c是允许的，但修改a1指向的内容是不允许的。
   // *a1 = 1;//error;    a1指向的内容不能修改。

   // a2 = &c;//error;    a2是int* const类型，const在int*的右侧，用来修饰指针本身，即指针本身为常量。所以修改指针本身是不允许的;但修改其内容是允许的。
   *a2 = 1;   //right
